...
df_avg = (
    df_core.groupby(group_cols, as_index=False)[avg_cols]
    .mean()
)
...
df_share["Age Total"] = df_share[age_cols].sum(axis=1)
...
df_ranked["max_age_share"] = df_ranked[age_cols].max(axis=1)
df_ranked["dominant_age_group"] = df_ranked[age_cols].idxmax(axis=1)

# Final 18 diagnosis categories selected after exploratory ranking and balancing across life stages
selected_codes = [
    "F89", "Q54", "E30", "N44", "J05", "L05", "O21", "N98", "O48",
    "N95", "D25", "N70", "C61", "C34", "C45", "G30", "F01", "R54"
]

df_final = df_ranked[df_ranked["Diagnosis Code"].isin(selected_codes)].copy()
...
plot_age_cols = [col for col in age_cols if col != "Age 0"]
